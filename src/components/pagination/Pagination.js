    ...
    return (
        <div className="App">
            <table className="table table-hover">
                ...
                {data.map(item => (
                    <tr key={item.id}>
                        <th scope="row"><img className="circle-img" src={item.imageURL}/>
                            <span>{item.symbol.toUpperCase()} <span>{item.name}</span> </span></th>
                        <td>$ {item.currentPrice}</td>
                        <td style={{color: item.priceChangePercentage.toString().slice(0, 1).includes('-') ? '#BF3F4D' : '#4AA372'}}>{item.priceChangePercentage.toString().slice(0, 5)}%</td>
